...
/// Jasmine/Karma and Playwright): it/test blocks, skipped (xit, .skip, .fixme),
/// focused (.only / fit, reported as "explicit") and commented-out tests.
/// </summary>
public static partial class JsTestScanner
{
    // it( / test( / fit( / xit( / it.skip( / test.only( / test.fixme( / it.each(...)( etc.
    // Playwright uses `test(...)`, `test.skip`, `test.fixme` and `test.only`, which fall
    // out of the same shapes; `describe`/hooks (beforeEach, ...) never match `\s*[(`].
